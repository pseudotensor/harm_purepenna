...
int init_vpot_user(int *whichcoord, int l, int i, int j, int k, FTYPE pr[][N2M][N3M][NPR], FTYPE *V, FTYPE *A)
{
...
{
  extern int vpot2field(FTYPE A[][N1M+SHIFT1][N2M+SHIFT2][N3M+SHIFT3],FTYPE pr[][N2M][N3M][NPR]);
  int i,j,k,pl;
  int toreturn;
...
int normalize_densities(FTYPE pr[][N2M][N3M][NPR])
{
  int i,j,k;
  FTYPE X[NDIM],V[NDIM],r,th;
  ...
  COMPZLOOP{
    ...
    if (pr[i][j][k][RHO] > rhomax)   rhomax = pr[i][j][k][RHO];
    if (pr[i][j][k][UU] > umax && r > rin)    umax = pr[i][j][k][UU];
  }
...
int normalize_field(FTYPE pr[][N2M][N3M][NPR])
{
  int i,j,k;
  FTYPE bsq_ij;
  ...
  COMPZLOOP {
	...
	if (bsq_calc(pr[i][j][k], &geom, &bsq_ij) >= 1)
...
int set_atmosphere(int whichcond, int whichvel, struct of_geom *ptrgeom, FTYPE *pr)
{
  ...
  if(rescaletype==4){
    ...
    prlocal[UU]=0.00069*pow(prlocal[RHO], gam)/(gam-1.);
#elif(WHICHPROBLEM==GRBJET)
    // couple rescaletype to atmosphere type
